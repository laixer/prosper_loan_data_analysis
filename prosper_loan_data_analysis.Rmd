...
```{r}
loans.completed_by_month <- loans %>%
...
ggplot(loans.completed_by_month, 
...
loans.completed_by_month2 <- loans %>%
  mutate(ProsperScore = as.factor(ProsperScore)) %>%
  filter(LoanStatus %in% c("Completed", "Chargedoff")) %>%
  filter(LoanOriginationMonth > "2009-07-01") %>%
  group_by(LoanOriginationMonth, LoanStatus) %>%
...
x <- merge(loans.completed_by_month, loans.completed_by_month2, 
           by.x = c("LoanOriginationMonth", "LoanStatus"))
x <- mutate(x, prop = NumLoansOriginated / NumLoansOriginatedTotal)

ggplot(x, 
        aes(LoanOriginationMonth, prop, fill = ProsperScore)) +
   facet_wrap(~ LoanStatus, ncol = 1) +
   geom_bar(stat = "identity") + 
   scale_x_datetime(labels = date_format("%b %Y"), 
                    breaks = date_breaks("3 months")) +
