...
```{r loans_over_quarter, cache=TRUE, fig.width=10, fig.height=6}
...
```{r loans_over_month, cache=TRUE, fig.width=10, fig.height=6}
...
```{r loans_by_month_state, cache=TRUE, fig.width=10, fig.height=6}
...
```{r loans_by_month_faceted_state, cache=TRUE, fig.width=10, fig.height=6}
ggplot(loans.originations_by_month_state, 
...
```{r loans_by_month_status, cache=TRUE, fig.width=10, fig.height=6}
...
```{r loans_by_month_status2, cache=TRUE, fig.width=10, fig.height=6}
loans.originations_by_month_status <- loans %>%
...
```{r loans_completed_prosper_score, cache=TRUE, fig.width=10, fig.height=6}
...
```{r loans_completed_by_month_normalized, cache=TRUE, fig.width=10, fig.height=6}
...
```{r, fig.width=10, fig.height=6}
# Returns a bar chart that shows proportions of loans by the given feature
# for charged-off loans of different terms.
plot_chargedoff_loans_term_breakdown <- function(feature) {
...
plot_chargedoff_loans_term_breakdown("ListingCategory")
```
...
```{r, fig.width=10, fig.height=6}
plot_chargedoff_loans_term_breakdown("Occupation")
```

Nothing interesting here. 

```{r, fig.width=10, fig.height=6}
plot_chargedoff_loans_term_breakdown("EmploymentStatus")
plot_chargedoff_loans_term_breakdown("IsBorrowerHomeowner")
plot_chargedoff_loans_term_breakdown("IncomeVerifiable")
```

```{r, fig.width=10, fig.height=6}
loans.completed$CreditScoreRangeLowerBucket = cut(
  loans.completed$CreditScoreRangeLower, breaks=10)
plot_chargedoff_loans_term_breakdown("CreditScoreRangeLowerBucket")
```

```{r, fig.width=10, fig.height=6}
plot_chargedoff_loans_term_breakdown("IncomeRange")
```

```{r, fig.width=10, fig.height=6}
loans.completed$LoanOriginalAmountBucket = cut(
  loans.completed$LoanOriginalAmount, breaks=20, dig.lab=8)
plot_chargedoff_loans_term_breakdown("LoanOriginalAmountBucket")
```
...
plot_completed_loans_categorical_breakdown("BorrowerAPRBucket")
```
